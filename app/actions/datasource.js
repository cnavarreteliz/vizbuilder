			...
			.then(
				(cubes) => {
					...
					if (params && params.cb) {
						...
						if (params.dd)
							set_cube_payload.level = main_cube.stdLevels.find((lv) => lv.key == params.dd);
						if (params.ms)
							set_cube_payload.measure = main_cube.measures.find((ms) => ms.key === params.ms);
					}

					if (!set_cube_payload.payload) set_cube_payload.payload = pickOne(new_cubes);
		...
		return client
			...
			.then(
				(request) => {
					 ...
					 * for that row.
					 */
					let values = flattenDrilldowns(request.data.axes, request.data.values);
